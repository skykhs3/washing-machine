...
export const CONFIG = {
  ...
  // The course rates are the real thing, so the stages are cut to fit them
  // rather than the other way round: `fillRate` takes 159 s to bring the drum
  // to 35 % and `drainRate` 78 s to empty it again, and every stage that asks
  // for one of those is that transfer plus a few seconds. Cycle leans on those
  // few seconds, so a stage has to stay at least as long as the transfer it
  // asks for. MANUAL hands the valve and the pump to the user, and waiting
  // minutes for a slider is not watching a machine, so there the two run
  // faster: the whole drum fills in about 30 s and empties in about 15, quick
  // enough that the slider answers and slow enough to watch it move.
  water: {
    fillRate: 0.0022,
    drainRate: 0.0045,
  ...
  // Standard cotton course, about 55 minutes: wash, two rinses with a short
  // spin after each drain, then a 10 minute final spin. A stage that only moves
  // water is as long as that transfer takes plus a few seconds, so the machine
  // is not left standing with the valve already shut.
